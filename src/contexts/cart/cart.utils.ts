...
export const calculateItemTotal = (items: Item[]) =>
  items.reduce((total, item) => total + item.quantity! * item.price, 0);

export const calculateSubTotal = (items: Item[]) => {
  const total = calculateItemTotal(items);
  return total;
};

export const calculateTotal = (items: Item[], coupon?: Coupon) => {
  const total = calculateItemTotal(items);

  const shipping = 0;

  if (coupon) {
    return Math.floor(getDiscount(total, coupon).total + shipping);
  }
